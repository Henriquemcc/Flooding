...
void RSUApplication::readDataFromFile() {
    ...
    for (int ID = 1; ID <= par("numberPackets").stringValue(); ID++) {
        MessageEntryInfo* videoInfo = new MessageEntryInfo;
        videoInfo->ID = ID;
...
WaveShortMessage*  RSUApplication::prepareWSM(std::string name, int lengthBits, t_channel channel, int priority, int rcvId, int serial) {
